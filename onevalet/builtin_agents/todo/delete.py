"""
Delete Todo Agent - Delete todo tasks

Multi-step agent for deleting tasks with search and approval using StandardAgent base class.

State Flow:
1. INITIALIZING -> extract fields, search tasks
2. WAITING_FOR_APPROVAL -> show found tasks, wait for user confirmation
...
class DeleteTodoAgent(StandardAgent):
    ...
    search_query = InputField(
        ...
        description="Search query for tasks to delete",
    )
    ...
    def get_approval_prompt(self) -> str:
        """Generate confirmation prompt with list of tasks to delete"""
    ...
    async def extract_fields(self, user_input: str) -> Dict[str, Any]:
        ...
        extraction_prompt = f"""Extract task deletion criteria from the user's request:
...
Rules:
- search_query: Keywords to identify which task(s) to delete.
  Extract the core task description from phrases like "delete X", "remove X task"."""
    ...
    async def on_initializing(self, msg: Message) -> AgentResult:
        ...
        if not self.found_tasks and not self.collected_fields.get("search_query"):
            return self.make_result(
                ...
                raw_message="Which task would you like to delete? Tell me the task name or keywords."
            )
    ...
    async def on_waiting_for_approval(self, msg: Message) -> AgentResult:
        ...
        if approval == "approved":
            ...
            return self.make_result(
                ...
                raw_message="Got it, I won't delete those tasks."
            )
            ...
            return self.make_result(
                status=AgentStatus.WAITING_FOR_INPUT,
                raw_message="Which task would you like to delete instead?"
            )
    ...
    async def _parse_approval_with_selection(self, user_response: str) -> str:
        ...
        prompt = f"""Parse the user's response to select which tasks to delete.
    ...
    async def on_running(self, msg: Message) -> AgentResult:
        ...
        if not self.found_tasks:
            return self.make_result(
                ...
                raw_message="I couldn't find any tasks to delete."
            )
        ...
        try:
            ...
            if deleted_count > 0 and failed_count == 0:
                return self.make_result(
                    status=AgentStatus.COMPLETED,
                    raw_message=f"Done! Deleted {deleted_count} task(s)."
                )
            elif deleted_count > 0 and failed_count > 0:
                return self.make_result(
                    status=AgentStatus.COMPLETED,
                    raw_message=f"Deleted {deleted_count} task(s), but {failed_count} failed."
                )
            else:
                return self.make_result(
                    status=AgentStatus.COMPLETED,
                    raw_message="I had trouble deleting those tasks. Want me to try again?"
                )
