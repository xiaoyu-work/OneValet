...
Multi-step agent for deleting emails with search and approval using StandardAgent base class.

State Flow:
1. INITIALIZING -> extract fields, search emails
2. WAITING_FOR_APPROVAL -> show found emails, wait for user confirmation
3. RUNNING -> execute deletion
4. COMPLETED
"""
import logging
import json
...
class DeleteEmailAgent(StandardAgent):
    ...
    account = InputField(
        ...
        description="Email account to delete from (default: primary)",
        required=False,
    )
    search_query = InputField(
        prompt="What emails would you like to delete?",
        description="Search query for emails to delete",
        required=False,
    )
    ...
    def get_approval_prompt(self) -> str:
        """Generate confirmation prompt with list of emails to delete"""
    ...
    async def extract_fields(self, user_input: str) -> Dict[str, Any]:
        ...
        extraction_prompt = f"""Extract email deletion criteria from the user's request:
...
{{
  ...
  "permanent": false
        ...
        try:
            ...
            self.permanent = extracted.get("permanent", False)
    ...
    async def on_initializing(self, msg: Message) -> AgentResult:
        ...
        if not self.found_emails and not self.collected_fields.get("search_query"):
            return self.make_result(
                ...
                raw_message="What emails would you like to delete? Tell me the sender, subject, or keywords."
            )
    ...
    async def on_waiting_for_approval(self, msg: Message) -> AgentResult:
        ...
        if approval == "approved":
            ...
            return self.make_result(
                ...
                raw_message="Got it, I won't delete those emails."
            )
            ...
            return self.make_result(
                status=AgentStatus.WAITING_FOR_INPUT,
                raw_message="What emails would you like to delete instead?"
            )
    ...
    async def _parse_approval_with_selection(self, user_response: str) -> str:
        ...
        prompt = f"""Parse the user's response to select which emails to delete.

Total emails: {email_count} (numbered 1 to {email_count})
User response: "{user_response}"

Determine intent:
- Delete ALL: {{"action": "all"}}
- Delete SPECIFIC: {{"action": "select", "indices": [1, 2, 3]}}
    ...
    async def on_running(self, msg: Message) -> AgentResult:
        ...
        if not self.message_ids:
            return self.make_result(
                ...
                raw_message="I couldn't find any emails to delete."
            )
        ...
        try:
            ...
            result = await provider.delete_emails(
                message_ids=self.message_ids,
                permanent=self.permanent
            )

            if result.get("success"):
                deleted_count = result.get("deleted_count", len(self.message_ids))
                action = "permanently deleted" if self.permanent else "moved to trash"
                return self.make_result(
                    status=AgentStatus.COMPLETED,
                    raw_message=f"Done! I've {action} {deleted_count} email(s)."
                )
            else:
                return self.make_result(
                    status=AgentStatus.COMPLETED,
                    raw_message=f"I had trouble deleting those emails. {result.get('error', '')}"
                )

        except Exception as e:
            logger.error(f"Failed to delete emails: {e}", exc_info=True)
            return self.make_result(
                status=AgentStatus.COMPLETED,
